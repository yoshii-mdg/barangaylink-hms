...
// BUG FIX: `supabaseAdmin` is NOT exported from supabase.js (it's server-only).
// The original code tried to import it here which caused a runtime crash.
// Instead we use the Express API endpoint to activate the user.
...
export default function AcceptInvitation() {
  ...
  useEffect(() => {
    const checkInvitation = async () => {
      try {
        ...
        // Invited users have role set in user_metadata by the invite endpoint
        ...
        setUserId(user.id);
        // Pre-fill if metadata includes names
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      // STEP 1: Set the password via Supabase Auth
      await updatePassword(password);

      // STEP 2: Activate profile via Express API (server uses service_role to bypass RLS)
      await adminApi.activateProfile(userId);

      // STEP 3: Success — sign out and redirect to login
  ...
  const header = (
    <>
      ...
      <nav className="flex justify-center flex-wrap items-center mt-6 text-white text-sm gap-x-1" aria-label="Breadcrumb">
        {BREADCRUMBS.map((item, i) => (
          <span key={`${item}-${i}`} className="flex items-center">
            {item === 'HOME' ? (
              ...
              <span aria-current={i === BREADCRUMBS.length - 1 ? 'page' : undefined}>{item}</span>
            )}
            {i < BREADCRUMBS.length - 1 && <span className="mx-2" aria-hidden="true">/</span>}
          </span>
        ))}
      </nav>
    </>
  ...
  if (step === STEPS.LOADING) {
    return (
      <AuthLayout header={header}>
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full border border-gray-100 text-center text-gray-500 flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-4 border-[#005F02] border-t-transparent rounded-full animate-spin" />
  ...
  if (step === STEPS.INVALID_LINK) {
    return (
      <AuthLayout header={header}>
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full border border-gray-100 text-center space-y-4">
          <p className="text-red-600 font-medium">This invitation link is invalid or has expired.</p>
          <p className="text-gray-500 text-sm">Invitation links are single-use and expire after 7 days.</p>
          <Link
            to="/login"
  ...
  const requirements = [
    ...
    { met: hasUppercase, label: 'At least one uppercase letter' },
    { met: passwordMatch, label: 'Passwords match' },
  ];

  return (
    <AuthLayout header={header}>
      <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8 w-full border border-gray-100 text-gray-900">
        ...
        <form onSubmit={handlePasswordSubmit} className="space-y-4">
          ...
          {/* Password */}
          <div>
            <label htmlFor="inv-password" className="block text-[#005F02] font-bold mb-2 text-base">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-gray-500 hover:text-[#005F02] focus:outline-none"
          ...
          {/* Confirm Password */}
          <div>
            <label htmlFor="inv-confirm-password" className="block text-[#005F02] font-bold mb-2 text-base">
          ...
          <div className="space-y-1.5 py-1">
            {requirements.map((req) => (
              <div key={req.label} className="flex items-center gap-2">
                <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${req.met ? 'bg-emerald-100' : 'bg-gray-100'}`}>
          ...
          {error && (
            <p role="alert" className="text-sm text-red-600 text-center bg-red-50 border border-red-200 rounded-lg px-4 py-2">
              {error}
            </p>
          )}
          ...
          <button
            type="submit"
            disabled={!canSubmit}
            className="w-full py-3 rounded-lg bg-[#005F02] text-white font-bold uppercase tracking-wide hover:bg-[#004A01] disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm mt-4"
          >
