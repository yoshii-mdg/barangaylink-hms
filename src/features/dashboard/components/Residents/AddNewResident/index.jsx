...
export default function ResidentAddEdit({ isOpen, onClose, onSubmit, initialData = null, mode = 'add' }) {
  const getInitialFormData = useMemo(() => {
    if (initialData && mode === 'edit') {
      // For simplicity, assume the name is stored as "LastName FirstName MiddleName Suffix"
      // But since the mock data might not follow this, we'll do basic parsing
      const nameParts = initialData.name?.split(' ') || [];
      let lastName = '', firstName = '', middleName = '', suffix = '';

      if (nameParts.length >= 2) {
        // Assume last part is lastName, first part is firstName, middle are middleName
