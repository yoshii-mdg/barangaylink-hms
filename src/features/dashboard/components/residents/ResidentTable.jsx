    ...
    <div className="overflow-x-auto w-full">
      <table className="w-full min-w-[700px] text-base">
        ...
        <tbody>
          {residents.length === 0 ? (
            <tr>
              <td colSpan={8} className="py-12 text-center text-gray-400 text-sm">No residents found.</td>
            </tr>
          ) : (
            residents.map((resident, idx) => (
              ...
              >
                ...
                <td className="py-3 px-4 relative">
                  <ActionDropdown
                    actions={[
                      { icon: HiPencilSquare,  label: 'Edit',             onClick: () => onEditResident?.(resident) },
                      { icon: RiUserMinusLine, label: 'Archive Resident', onClick: () => onArchiveResident?.(resident) },
                      { icon: FaRegTrashAlt,   label: 'Delete',           onClick: () => onDeleteResident?.(resident), className: 'text-red-600 hover:text-red-700 hover:bg-red-50' },
                    ]}
                  />
                </td>
              </tr>
